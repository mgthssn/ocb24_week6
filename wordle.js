...
const hasNotLettersInPosition = (word, letter, position) => {
  if (letter === undefined) return true;
  return word[position] !== letter;
}
...
const filteredWords = allWords.filter(word => {
  ...
  mustNotHaveLettersInPosition.every((letter,index) => hasNotLettersInPosition(word, letter, index))
})
